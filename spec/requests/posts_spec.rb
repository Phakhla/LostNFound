...
RSpec.describe 'Posts', type: :request do
  ...
  describe 'GET /posts/lost' do
    it 'gets lost page' do
      ...
      expect(response.body).to include('<h4 class="col-auto">รายการโพสต์ตามหาของทั้งหมด(0)</h4>')
      expect(response).to have_http_status(:ok)
    end
  ...
  describe 'GET /posts/found' do
    it 'gets found page' do
      ...
      expect(response.body).to include('<h4 class="col-auto">รายการโพสต์ตามหาเจ้าของทั้งหมด(1)</h4>')
      expect(response).to have_http_status(:ok)
    end
